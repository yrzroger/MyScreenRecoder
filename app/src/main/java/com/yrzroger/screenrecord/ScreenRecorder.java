...
public class ScreenRecorder {
    ...
    public void start() {
        ...
        Log.i(TAG, "sendEmptyMessage(MSG_START)");
    }
    ...
    private void record() {
        ...
        if (VERBOSE) Log.i(TAG, "created virtual display: " + mVirtualDisplay.getDisplay());
    }
    ...
    private void writeSampleData(int track, MediaCodec.BufferInfo buffer, ByteBuffer encodedData) {
        ...
        if (buffer.size == 0 && !eos) {
            ...
            if (!eos && mCallback != null) {
                if(mAudioTrackIndex != INVALID_INDEX && track == mAudioTrackIndex)
                    mCallback.onRecording(buffer.presentationTimeUs);
                else if(mAudioTrackIndex == INVALID_INDEX  && track == mVideoTrackIndex)
                    mCallback.onRecording(buffer.presentationTimeUs);
            }
        }
        if (encodedData != null) {
